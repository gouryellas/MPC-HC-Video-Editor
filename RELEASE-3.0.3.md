...
agree. The color does not change with the label: it is the same command doing
the same kind of work, and a button that changed color as cuts were ticked
...
than a fixed behavior. It is on by default, so nothing changes unless you turn
