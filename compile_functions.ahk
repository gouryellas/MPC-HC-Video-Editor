edit_bookmarks(var:="") {
	...
	show_steps:
		...
		Menu, MyMenuBar, Add, Merge, merge
		Menu, MyMenuBar, Add, Split, split
		Gui, Menu, MyMenuBar
			...
			if (a_loopfield != "") {
				if (pretty_print != "") {
					...
					if regexmatch(a_loopfield, "imO)(.+)\s\-\s(.+)\s(.+)", split) {
						...
						gui, add, link, gseek_steps x+80 +center, <a id="a">%time_split1%</a> - <a id="b">%time_split2%</a> %duration_split%		
					} else {
						global time_split3 := a_loopfield
						save_latest_time := time_split3
						gui, add, link, gseek_steps x+80 , <a id="c">%time_split3%</a>
					}
					buttonCount++
		...
		if (bookmark_status = "complete") {
			ui_add_text("cut length: " . time_secToAlt(edited_duration) . " / " . time_total, "x+80")
			save_duration := edited_duration
		} else if (line_num = "1")
			ui_add_text("video duration: " . time_total, "x+70")
		else
			ui_add_text("cut length: " . time_secToAlt(save_duration) . " / " . time_total, "x+80")
		xpos := a_screenwidth - 1000
		gui, show, NA y140 x%xpos% w330, MPC Video Editor v1.3
		if (timer = 1)
			gosub, seek_steps
	return
	...
	guiclose:
		window_kill("MPC-BE")
		window_kill("MPC-BE Video Editor v1.3")
		ui_destroy("basic")
		ui_destroy("basic2")
	return
	...
	split:
		if (bookmark_status = "complete") {
			msgbox, 4, , Would you like to delete the original file?
			ifmsgbox, yes
				{
				selectedDelete = 1
			}
			...
			msg("You cannot split files without completing the last bookmark.")
	return	
	...
	merge:
		if (bookmark_status = "complete") {
			...
			run(a_temp . "\compile_merge.bat" . file_path_create_done . ffmpeg_time)
			window_waitActive("ahk_exe cmd.exe")
			window_waitNotActive("ahk_exe cmd.exe")
			if (selectedDelete = 1)
				file_recycle(path_join(file_directory, file_name, file_extension_dot))
			...
			msg("You cannot merge files without completing the last bookmark.")
	return
...
{
    Gui, basic:Destroy
    ; put any static fonts/colors here if needed
}
